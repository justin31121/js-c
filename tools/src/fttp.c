...
int main() {
	...
	if(ip_sockets_register(&sockets, HTTPSERVER_SOCKETS_COUNT - 1) != IP_ERROR_NONE) {
		return 1;
	}
	...
	event.data.ptr = (void *) (HTTPSERVER_SOCKETS_COUNT + FTPSERVER_SOCKETS_COUNT - 1); 
	if(epoll_ctl(sockets.epfd, 
			EPOLL_CTL_ADD, 
			sockets.sockets[HTTPSERVER_SOCKETS_COUNT + FTPSERVER_SOCKETS_COUNT - 1]._socket, 
			&event) != 0) {
		TODO();
	}
