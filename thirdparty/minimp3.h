...
static uint32_t mp3_get_bits(bs_t *bs, int n)
{
...
static void L12_read_scale_info(const uint8_t *hdr, bs_t *bs, L12_scale_info *sci)
{
    ...
    for (i = 0; i < sci->total_bands; i++)
    {
        ...
        ba = ba_code_tab[mp3_get_bits(bs, ba_bits)];
        sci->bitalloc[2*i] = ba;
    ...
    {
        sci->scfcod[i] = sci->bitalloc[i] ? HDR_IS_LAYER_1(hdr) ? 2 : mp3_get_bits(bs, 2) : 6;
    }
        ...
        for (i = 0; i < 2*sci->total_bands; i++)
        {
            ...
            if (ba != 0)
            {
                if (ba < 17)
                {
                    ...
                    {
                        dst[k] = (float)((int)mp3_get_bits(bs, ba) - half);
                    }
                } else
                {
                    unsigned mod = (2 << (ba - 17)) + 1;    /* 3, 5, 9 */
                    unsigned code = mp3_get_bits(bs, mod + 2 - (mod >> 3));  /* 5, 7, 10 */
...
static int L3_read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    {
        gr_count *= 2;
        main_data_begin = mp3_get_bits(bs, 9);
        scfsi = mp3_get_bits(bs, 7 + gr_count);
    } else
    {
        main_data_begin = mp3_get_bits(bs, 8 + gr_count) >> gr_count;
    }
    ...
    do
    {
        ...
        gr->part_23_length = (uint16_t)mp3_get_bits(bs, 12);
        part_23_sum += gr->part_23_length;
        gr->big_values = (uint16_t)mp3_get_bits(bs,  9);
        if (gr->big_values > 288)
        {
            return -1;
        }
        gr->global_gain = (uint8_t)mp3_get_bits(bs, 8);
        gr->scalefac_compress = (uint16_t)mp3_get_bits(bs, HDR_TEST_MPEG1(hdr) ? 4 : 9);
        gr->sfbtab = g_scf_long[sr_idx];
        gr->n_long_sfb  = 22;
        gr->n_short_sfb = 0;
        if (mp3_get_bits(bs, 1))
        {
            gr->block_type = (uint8_t)mp3_get_bits(bs, 2);
            if (!gr->block_type)
            {
                return -1;
            }
            gr->mixed_block_flag = (uint8_t)mp3_get_bits(bs, 1);
            gr->region_count[0] = 7;
            ...
            tables = mp3_get_bits(bs, 10);
            tables <<= 5;
            gr->subblock_gain[0] = (uint8_t)mp3_get_bits(bs, 3);
            gr->subblock_gain[1] = (uint8_t)mp3_get_bits(bs, 3);
            gr->subblock_gain[2] = (uint8_t)mp3_get_bits(bs, 3);
        } else
        {
            gr->block_type = 0;
            gr->mixed_block_flag = 0;
            tables = mp3_get_bits(bs, 15);
            gr->region_count[0] = (uint8_t)mp3_get_bits(bs, 4);
            gr->region_count[1] = (uint8_t)mp3_get_bits(bs, 3);
            gr->region_count[2] = 255;
        }
        gr->table_select[0] = (uint8_t)(tables >> 10);
        gr->table_select[1] = (uint8_t)((tables >> 5) & 31);
        gr->table_select[2] = (uint8_t)((tables) & 31);
        gr->preflag = HDR_TEST_MPEG1(hdr) ? mp3_get_bits(bs, 1) : (gr->scalefac_compress >= 500);
        gr->scalefac_scale = (uint8_t)mp3_get_bits(bs, 1);
        gr->count1_table = (uint8_t)mp3_get_bits(bs, 1);
    ...
    for (i = 0; i < 4 && scf_count[i]; i++, scfsi *= 2)
    {
        ...
        } else
        {
            ...
            } else
            {
                ...
                for (k = 0; k < cnt; k++)
                {
                    int s = mp3_get_bits(bitbuf, bits);
